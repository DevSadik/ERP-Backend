...
// ── Barcode Lookup (own → central → registry) ─────────────────────────────────
export const lookupBarcode = async (req, res, next) => {
  try {
    ...
    let product = await Product.findOne({
      shop: shopId, isActive: true,
      $or: [{ barcode: code }, { sku: code.toUpperCase() }],
    });
    if (product) return ok(res, { ...product.toJSON(), source: 'own' });

    // 2. Central DB — Option A: only expose name/company/category/mrp
    const central = await Product.findOne({
      isCentral: true, isActive: true,
      $or: [{ barcode: code }, { sku: code.toUpperCase() }],
    });
    if (central) return ok(res, {
      _id: null, name: central.name, company: central.company,
      category: central.category, barcode: central.barcode,
      unit: central.unit, mrp: central.mrp, salePrice: central.mrp,
      costPrice: '', source: 'central',
    });
...
export const createSale = async (req, res, next) => {
  try {
    ...
    const sale = await StockOut.create({
      shop: shopId, orderId, items,
      customer: customer || 'Walk-in',
      paymentType, paymentStatus,
      totalAmount: +totalAmount, discount: +discount || 0, notes,
    });
    ...
    if (paymentType === 'credit') {
      await CreditLedger.create({
        ...
        transactionType: 'credit', amount: +totalAmount,
        balance: +totalAmount, reference: orderId,
      });
    }
